...
export const makeRichText = (answer, options, onValueChanged = () => { }) => {
    const { baseUrl, fileTypes, sanitize, screenshotSaver, ignoreSaveObject, screenshotImageSelector, invalidImageSelector, locale, updateMathImg, } = Object.assign(Object.assign({}, u.defaults), options);
    ...
    $(answer)
        ...
        if (richTextAndMathBlur())
            onRichTextEditorBlur(state.$currentEditor);
    })
        .on('focus blur', (e) => {
        if (e.type === 'focus')
            state.math.closeMathEditor();
        onRichTextEditorFocusChanged(e);
    })
        // Triggered after both drop and paste
        .on('input', (e) => {
        if (!pasteInProgress)
            onValueChanged(ignoreSaveObject || u.sanitizeContent(e.currentTarget, screenshotImageSelector, sanitize));
    })
        .on('drop', (e) => {
...
function onRichTextEditorFocusChanged(e) {
    ...
    richTextEditorBlurTimeout = setTimeout(() => {
        if (!helpOverlayOpen() && richTextAndMathBlur())
