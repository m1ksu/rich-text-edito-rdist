...
export function persistInlineImages($editor, screenshotSaver, invalidImageSelector, fileTypes) {
    setTimeout(() => Promise.all(markAndGetInlineImagesAndRemoveForbiddenOnes($editor, invalidImageSelector, fileTypes).map((data) => screenshotSaver(data)
        .then((screenShotUrl) => data.el.setAttribute('src', screenShotUrl))
        .catch((err) => {
        data.el.remove();
        throw err;
    }))).then(() => $editor.trigger('input')), 0);
}
function markAndGetInlineImagesAndRemoveForbiddenOnes($editor, invalidImageSelector, fileTypes) {
    ...
    const images = $editor
        ...
        .toArray()
        .map((el) => (Object.assign(Object.assign({}, decodeBase64Image(el.getAttribute('src'))), { el })));
    images
